        ...
        {data?.map((ranking, index) => (
          ...
          >
            ...
            <Avatar className="h-10 w-10">
              <AvatarImage
                src={`https://api.dicebear.com/6.x/initials/svg?seed=${ranking.firstName}`}
              />
              <AvatarFallback>{ranking.firstName.charAt(0)}</AvatarFallback>
            </Avatar>
            <div className="flex-grow">
              <p className="font-semibold">{ranking.firstName}</p>
              <p className="text-sm text-gray-400">Level {ranking.xp?.level}</p>
            </div>
