...
import { Coins, Zap, ArrowRight, User } from 'lucide-react';
import {
	Tabs,
	TabsContent,
	TabsList,
...
export function InGameExchange() {
	...
	const [response, formAction] = useFormState(
		executePowerExchange.bind(null, telegramId),
		{
			message: {},
		}
	);
	...
	return (
		<Card className='w-full max-w-md bg-gray-800/30 border-2 border-blue-400/50 text-white shadow-lg shadow-blue-500/20 backdrop-blur-md rounded-xl'>
			<CardContent className='space-y-6 p-6'>
				<div className='flex justify-between text-sm text-blue-300 mt-4'>
					<span>Available Coins: {profile.balance}</span>
				...
				>
					...
					<TabsContent value='buyPasses' className='mt-4 space-y-4'>
						<ExchangeContent
							...
							maxPasses={Math.floor(profile.balance / 20)}
						/>
					</TabsContent>
				...
				<form action={formAction}>
					...
					/>
					<SubmitButton title='Exchange Now' loadingTitle='Converting...' />
				</form>
			</CardContent>
		</Card>
	);
