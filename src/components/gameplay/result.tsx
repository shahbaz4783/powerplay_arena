    ...
    <div className="w-full bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl shadow-2xl">
      ...
      <section className="space-y-6 p-6">
        ...
        <Card className={`${getResultColor()} border-none`}>
          <CardContent className="pt-6">
            <h3
              className={`text-2xl font-bold text-center ${matchResult.winner === "tie" ? "text-yellow-400" : matchResult.winner === "player" ? "text-green-400" : "text-red-400"}`}
            >
              {getResultText()}
            </h3>
            <p className="text-slate-300 mt-2 text-center">
              {matchResult.winner === "tie"
                ...
                : `${matchResult.winner === "player" ? "You" : "Opponent"} won by ${matchResult.margin} ${matchResult.marginType}`}
            </p>
          </CardContent>
        </Card>
